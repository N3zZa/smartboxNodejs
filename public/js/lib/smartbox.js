  ...
  var PlatformApi = {
    ...
    getRandomDUID: function () {
      return (
        ...
        Math.floor(Math.random() * parseInt("10000", 16)).toString(16)
      );
    },
...
(function ($, window, document, undefined) {
  var pluginName = "SBInput",
    defaultOptions = {
      ...
      input: {
        template:
          '<div class="smart_input-container">' +
  ...
  function generateKeyboardPopup() {
    $keyboardOverlay = $(document.createElement("div")).attr(
      "id",
      "keyboard_overlay"
    );
  ...
  pluginPrototype = {
    ...
    addEvents: function () {
      ...
      if (opt.directNumInput && !opt.directKeyboardInput) {
        this.$el
          .off("nav_key:num nav_key:red")
          .on("nav_key:num nav_key:red", function (e) {
            self.typeNum(e);
          });
      }
    ...
    setText: function (text) {
      ...
      if (text.length > 1) {
        ...
        this.$wrapper[method + "Class"](
          "." + opt.input.wrapperClass + "_right"
        );
      } else {
    ...
    type: function (letter) {
      ...
      if (text.length === opt.max && opt.next && opt.max != 0) {
        this.hideKeyboard();
    ...
    showKeyboard: function () {
      ...
      $keyboardPopup
        .SBKeyboard("changeKeyboard", keyboardOpt.type)
        .css({
          left: o.left,
          top: top,
        })
...
(function ($, window, document, undefined) {
  ...
  keyboardPrototype = {
    ...
    initialize: function (options) {
      ...
      this.$wrap.append(board).addClass("kekekey_" + _type);
      ...
      if (haveNums) {
        this.$layouts["fullnum"] = this.$wrap.find(
          ".keyboard_generated_fullnum"
        );
      }
    ...
    generateBoard: function (type) {
      ...
      for (var i = 0; i < preset.length; i++) {
        ...
        for (var j = 0; j < row.length; j++) {
          ...
          rowHtml +=
            '<div class="kbtn nav-item ' +
            keyAttrs.type +
            '" data-letter="' +
            _.escape(keyAttrs.letter) +
            '"';
    ...
    show: function () {
      this.$wrap.show();
      this.$el
        .addClass(_.result(this, "type") + "_wrap")
        .addClass("keyboard_" + this.currentLayout);
      return this;
    },
    hide: function () {
      this.$wrap.hide();
      this.$el
        .removeClass(_.result(this, "type") + "_wrap")
...
(function (window, undefined) {
  ...
  var logPanelTemplate =
    '<div class="log_pane" id="log_<%=name%>">' +
    ...
    stopProfile: function (profileName) {
      if (profiles[profileName]) {
        this.log(
          profileName +
            ": " +
            (new Date().getTime() - profiles[profileName]) +
            "ms",
          "profiler"
        );
...
!(function (window, undefined) {
  ...
  function Navigation() {
    ...
    function throttleEvent(key) {
      ...
      // lazy init
      if (!keyMethod) {
        keyMethod = throttledMethods[key] = _.throttle(
          function () {
            triggerKeyEvent(key);
          },
          800,
    ...
    return {
      ...
      current: function (element, originEvent) {
        ...
        $el
          .addClass(this.higlight_class)
          .trigger("nav_focus", [originEvent, old]);
        return this;
      },
      ...
      siblingsTypeNav: function ($container, type, loop) {
        var self = this;
        $container.on(
          "nav_key:left nav_key:right nav_key:up nav_key:down",
          this.area_selector,
          function (e) {
            var last = "last",
              cur = self.current(),
            ...
            if (
              (type == "hbox" && e.keyName == "left") ||
              (type == "vbox" && e.keyName == "up")
            ) {
              fn = "prev";
            } else if (
              (type == "hbox" && e.keyName == "right") ||
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            var setSubtitlesText = function () {
              $("#subtitles_text").html(
                Player.subtitle.text(
                  parseInt(Player.videoInfo.currentTime) * 1000
                )
              );
            };
            ...
            if (!$subtitiles.length) {
              $("body").append(
                '<div id="subtitles_view" style="position: absolute; z-index: 1;"><div id="subtitles_text"></div></div>'
              );
              ...
              $("#subtitles_text").css({
                ...
                color: "#fff",
                "text-shadow":
                  "0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000,0 0 3px #000",
                "line-height": "26px",
              });
            }
      ...
      parse: {
        smi: function (data) {
          ...
          Player.subtitle.data = _.map(data, function (value) {
            var match =
              /[\s\S]*start=(\d+)[\s\S]*<p[^>]*>([\s\S]*)<spanid/i.exec(value);
            if (match) {
              return {
...
(function ($) {
  ...
  window.$$voice = {
    ...
    save: function () {
      if (currentVoiceState) stack.push(currentVoiceState);
      return this;
    },
    restore: function () {
      var last = stack.pop();
      if (last) $.fn.voiceLink.apply(last.self, last.args);
      return this;
    },
  ...
  var generateHelpBar = function (options) {
    ...
    if (activeItems.length > helpbarMaxItems) {
      activeItems.splice(helpbarMaxItems - 1, 0, {
        ...
        commandList: [{ command: opts.moreText }],
      });
    ...
    _.each(candidateVoiceItems, function (val) {
      ...
      if (!hidden) {
        ...
        if ($groupWrap.length) {
          ...
          $buble.append(
            '<div class="voice_group_head">' +
              group +
              "</div>" +
              '<div class="voice_group_body" id="voice_group_body_' +
              groupNames[group] +
              '">' +
              '<div class="voice_help_item">' +
              commands +
              "</div>" +
              "</div>"
          );
...
SB.readyForPlatform("browser", function () {
  Player.extend({
    _init: function () {
      ...
      this.$video_container.on(
        "abort canplay canplaythrough canplaythrough durationchange emptied ended error loadeddata loadedmetadata loadstart mozaudioavailable pause play playing ratechange seeked seeking suspend volumechange waiting",
        function (e) {
          //console.log(e.type);
        }
      );
    ...
    subtitle: {
      set: function (index) {
        if (Player.$video_container[0].textTracks) {
          var subtitles = _.filter(
            Player.$video_container[0].textTracks,
            function (i) {
  ...
  SB.readyForPlatform("browser", function () {
    _.extend($$voice, {
      _init: function () {
        ...
        $("body").append(
          '<div id="voice_buble"></div><div id="help_voice_bubble"></div><div class="emul_voice_helpbar_wrap"><div id="emul_voice_helpbar"></div></div>'
        );
        // клики по кнопкам эмулятора голоса
        $("#emul_voice_helpbar").on(
          "click",
          ".emul_voice_trigger",
          function () {
...
SB.readyForPlatform("lg", function () {
  ...
  Player.extend({
    ...
    _init: function () {
      var self = this;
      $("body").append(
        '<object type="video/mp4" data="" width="1280" height="720" id="pluginPlayer" style="z-index: 0; position: absolute; left: 0; top: 0;"></object>'
      );
  ...
  SB.readyForPlatform("samsung", function () {
    ...
    _.extend($$voice, {
      _init: function () {
        ...
        $("body").append(
          '<div id="voice_buble"></div><div id="help_voice_bubble"></div>'
        );
      },
      _nativeCheckSupport: function () {
        var enabled = false;
      ...
      _nativeFromServer: function (title, callback) {
        ...
        deviceapis.recognition.SetVoiceHelpbarInfo(
          JSON.stringify(describeHelpbar)
        );
      },
      _setVoiceHelp: function (voicehelp) {
